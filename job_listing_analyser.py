...
time.sleep(1)
print("\nNext we need a country code. Common examples are: in, fr, de")
user_coun = input("Please enter a country code: ")

time.sleep(1)
print("\nNext we need the city you want to search in (e.g. India, France)")
...
def search(search_term, country, city, number_pages=10):
    ...
    while m <= (number_pages * 10):
        ...
        csv_filename = '/Users/AnudeepReddy/Documents/Job_Scraper/' + city + '_' + search_term + '_' + current_date + '.csv'
...
def get_ran_postings(filename, num_postings, user_st):
    ...
    now = datetime.now()
    current_date = now.strftime("%d%m%Y")
    csv_filename = '/Users/AnudeepReddy/Documents/Job_Scraper/job_descriptions_' + current_date + '_parsed.csv'
    df.to_csv(csv_filename, index=False, header=True)
    return csv_filename
...
def parse_custom(filename, search_terms):
    ...
    #Printing to CSV
    csv_filename = '/Users/AnudeepReddy/Documents/Job_Scraper/keyword_results.csv'
